...
class MVAN(object):
  def __init__(self, hparams):
    ...
    random.seed(hparams.random_seed[0])
  ...
  def _build_model(self):
    ...
    if "disc" in self.hparams.decoder:
      ...
      self.margin_rank_loss_gt = nn.MarginRankingLoss(margin=self.hparams.margin_1)
      self.margin_rank_loss_random = nn.MarginRankingLoss(margin=self.hparams.margin_2)
  ...
  def _loss_fn(self, epoch, batch, output):
    ...
    batch_loss = self.criterion(output.view(-1, output.size(-1))/self.hparams.T, target.view(-1).to(self.device))

    return batch_loss
  def hinge_loss_final_rank(self,batch, output, margin_1 = 0.05, margin_2 = 0.5):
      fake_label_logit = batch['fake_label_logit'].cuda()
      fake_label_logit = fake_label_logit.view(-1, 100)
      output = output.view(-1, 100) / self.hparams.T
      scores = F.softmax(output, dim=-1)
      _, rank_ids = torch.sort(fake_label_logit, dim=-1)

      negetive_label = rank_ids[:, :-self.hparams.K]
      fake_labels = rank_ids[:, -(self.hparams.K+self.hparams.M):-self.hparams.K]
      similar_label = rank_ids[:, -self.hparams.K:]
      random_labels = rank_ids[:, 0:(100-self.hparams.K-self.hparams.M)]
      shuffle_labels = []
      for i in range(random_labels.size(0)):
          cur_labels = random_labels[i].tolist()
          random.shuffle(cur_labels)
          shuffle_labels.append(cur_labels)
      shuffle_label = torch.tensor(shuffle_labels).cuda()

      # shuffle_label = random_labels
      fake_labels_score = torch.gather(scores, dim=-1, index=fake_labels)
      random_labels_score = torch.gather(scores, dim=-1, index=shuffle_label[:, 0:self.hparams.N])
      smilar_score = torch.gather(scores, dim=-1, index=similar_label)
      negetive_label_score = torch.gather(scores, dim=-1, index=negetive_label)

      negetive_loss = torch.nn.functional.relu(self.hparams.margin_1-smilar_score.unsqueeze(1)+negetive_label_score.unsqueeze(2))
      fake_loss_1 = torch.nn.functional.relu(self.hparams.margin_2-fake_labels_score.unsqueeze(1)+random_labels_score.unsqueeze(2))

      loss = torch.mean(negetive_loss, [1, 2])
      loss_1 = torch.mean(fake_loss_1, [1,2])
      loss = loss + loss_1
  ...
  def train(self):
    ...
    for epoch in range(self.start_epoch, self.hparams.num_epochs):
      ...
      for i, batch in enumerate(tqdm_batch_iterator):
        ...
        output, _ = self.model(buffer_batch)

        ce_loss = self._loss_fn(epoch, batch, output)
        ranking_loss = self.hparams.alpha*self.hinge_loss_final_rank(batch, output, self.hparams.margin_1, self.hparams.margin_2)
        batch_loss = ce_loss + ranking_loss
        ...
        if self.hparams.virtual_batch_size == accumulate_batch \
          ...
          description = "[{}][Epoch: {:2d}][Iter: {:4d}][Loss: {:4f}][ce_loss: {:4f}][ranking_loss: {:4f}][lr: {:4f}]".format(
            datetime.utcnow() - train_begin,
            epoch,
            global_iteration_step, batch_loss,ce_loss, ranking_loss,
