...
class MultiMVAN(object):
  ...
  def hinge_loss_all_softmax(self, batch, output, margin_1=0.05, margin_2=0.5):
    fake_label_logit = batch['fake_label_logit'].cuda()
    fake_label_logit = fake_label_logit.view(-1, 100)
    target = batch['ans_ind'].view(-1).cuda()
    output = output.view(-1, 100) / self.hparams.T
    scores = F.softmax(output, dim=-1)
    # scores = output
    _, rank_ids = torch.sort(fake_label_logit, dim=-1)
    # fake_labels = rank_ids[:, -1 - self.hparams.sample_num:-1]

    # start = self.hparams.fake_label_num
    # end = 99
    # mid = int((start + end)/2)

    negetive_label = rank_ids[:, :-1]
    fake_label_1 = rank_ids[:, self.hparams.fake_label_start:-1]
    # fake_label_2 = rank_ids[:, start:mid]
    random_labels = rank_ids[:, 0:self.hparams.random_label_end]
    # random_nums = torch.randint(self.hparams.sample_range[0], self.hparams.sample_range[1],
    #                                         (fake_label_logit.size(0), self.hparams.sample_num)).cuda()
    # random_labels = torch.gather(rank_ids, dim=-1, index=random_nums)
    # fake_labels_score = torch.gather(scores, dim=-1, index=fake_labels)
    # random_labels_score = torch.gather(scores, dim=-1, index=random_labels)
    gt_score = torch.gather(scores, dim=-1, index=target.unsqueeze(1))
    negetive_label_score = torch.gather(scores, dim=-1, index=negetive_label)
    fake_label_1_score = torch.gather(scores, dim=-1, index=fake_label_1)
    # fake_label_2_score = torch.gather(scores, dim=-1, index=fake_label_2)
    random_labels_score = torch.gather(scores, dim=-1, index=random_labels)

    negetive_loss = torch.nn.functional.relu(margin_1 - gt_score + negetive_label_score)
    fake_loss_1 = torch.nn.functional.relu(
      margin_2 - fake_label_1_score.unsqueeze(1) + random_labels_score.unsqueeze(2))
    # fake_loss_2 = torch.nn.functional.relu(margin_2 - fake_label_2_score.unsqueeze(1) + random_labels_score.unsqueeze(2))
    loss = torch.mean(negetive_loss, dim=-1) + torch.mean(fake_loss_1, [1, 2])
    # loss = torch.mean(fake_loss_1, [1,2])
  ...
  def train(self):
    ...
    for epoch in range(self.start_epoch, self.hparams.num_epochs):
      ...
      for i, batch in enumerate(tqdm_batch_iterator):
        ...
        ranking_loss = self.hparams.alpha * self.hinge_loss_all_softmax(batch, disc_output, self.hparams.margin_1,
                                                                        self.hparams.margin_2)
        batch_loss = disc_loss + gen_loss + ranking_loss
        batch_loss.backward()
        ...
        if self.hparams.virtual_batch_size == accumulate_batch \
          ...
          description = "[{}][Epoch: {:3d}][Iter: {:4d}][Loss: {:4f}][Disc: {:4f}][Gen: {:4f}][rank: {:4f}][lr: {:7f}]".format(
            datetime.utcnow() - train_begin,
            epoch,
            global_iteration_step, running_loss, disc_loss.item(), gen_loss.item(), ranking_loss,
